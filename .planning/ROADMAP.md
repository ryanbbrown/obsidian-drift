...
This milestone hardens the existing diff viewer plugin into a reliable, configurable tool. The journey starts by verifying and committing the current functionality (testing), then makes the plugin configurable (settings), refactors rendering to be incremental (performance), adds diff persistence across reloads (the single biggest missing feature), and finishes by stripping dev-only commands for a clean production build.
...
- [ ] **Phase 4: Diff Persistence** - Persist pending diffs across Obsidian reloads with staleness validation and write-time safety checks
...
### Phase 4: Diff Persistence
**Goal**: Pending diffs survive Obsidian restarts and are safe to apply even when files have changed since the diff was generated
**Depends on**: Phase 3
**Requirements**: PERS-01, PERS-02, PERS-03
**Success Criteria** (what must be TRUE):
  1. User closes and reopens Obsidian and all pending diffs reappear in the diff view with correct content
  2. If a file was deleted or its content reverted to match the original snapshot while Obsidian was closed, the stale diff is silently discarded on reload
  3. When the user clicks accept/reject, the plugin re-reads the file first and warns if the file has changed since the diff was generated (no silent data loss)
**Plans**: TBD
